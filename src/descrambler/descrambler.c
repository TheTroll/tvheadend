...
descrambler_service_start ( service_t *t )
{
  ...
  if (t->s_descramble == NULL) {
    ...
    for (i = 0; i < DESCRAMBLER_MAX_KEYS; i++) {
      ...
      tvhcsa_init(&tk->key_csa, (mpegts_service_t *) t);
      if (!multipid) break;
    }
...
descrambler_service_stop ( service_t *t )
{
  ...
  if (dr) {
    for (i = 0; i < DESCRAMBLER_MAX_KEYS; i++) {
      tk = &dr->dr_keys[i];
      tvhcsa_destroy(&tk->key_csa, (mpegts_service_t *)t);
                   ...
                   const uint8_t *even, const uint8_t *odd )
{
  ...
  if (tvhcsa_set_type(&tk->key_csa, type) < 0) {
    ...
    tvhcsa_destroy(&tk->key_csa, (mpegts_service_t *) td->td_service);
    tvhcsa_init(&tk->key_csa, (mpegts_service_t *) td->td_service);
    if (tvhcsa_set_type(&tk->key_csa, type) < 0)
      goto end;
    tk->key_valid = 0;
  }
