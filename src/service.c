   ...
   int *error, int weight, int flags, int timeout, int postpone)
{
  ...
  if (ch) {
    ...
    /* UHD->HD+->FHD->HD->SD fallback */
    if (si == NULL && pro &&
        pro->pro_svfilter == PROFILE_SVF_UHD) {

      LIST_FOREACH(ilm, &ch->ch_services, ilm_in2_link) {
        s = (service_t *)ilm->ilm_in1;
        if (s->s_is_enabled(s, flags) && service_is_hdtv(s, 1)) {
          r1 = s->s_enlist(s, ti, sil, flags, weight);
          if (r1 && r == 0)
            r = r1;
        }
...
service_servicetype_txt ( service_t *s )
{
  static const char *types[] = {
    "HDTV", "HDTVPLUS", "FHDTV", "SDTV", "Radio", "UHDTV", "Other"
  };
  if (service_is_hdtv(s, 0))  return types[0];
  if (service_is_hdtv(s, 1))  return types[1];
  if (service_is_fhdtv(s)) return types[2];
  if (service_is_sdtv(s))  return types[3];
  if (service_is_radio(s)) return types[4];
