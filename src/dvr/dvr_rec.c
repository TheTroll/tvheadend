...
dvr_rec_subscribe(dvr_entry_t *de)
{
  ...
  if (aa->aa_muxes_limit_dvr)
  {
    ...
    if (ilm)
    {
      ...
      if (ch_first_service)
      {
        ...
        {
          tvherror(LS_DVR, "user [%s] is already using %d muxes for recording while the max is %d", de->de_creator?:"no-user", count, aa->aa_muxes_limit_dvr);
          access_destroy(aa);
          return -EINVAL;
        }
  ...
  de->de_s = subscription_create_from_channel(prch, NULL, weight,
					      buf, prch->prch_flags,
					      NULL, 0, aa->aa_username ?: "", NULL, NULL);
  if (de->de_s == NULL) {
