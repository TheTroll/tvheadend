 ...
 *
 * FFmpeg is free software; you can redvdpauribute it and/or
 * modify it under the terms of the GNU Lesser General Public
 * License as published by the Free Software Foundation; either
 * version 2.1 of the License, or (at your option) any later version.
 *
 * FFmpeg is dvdpauributed in the hope that it will be useful,
...
static void vdpau_uninit(AVCodecContext *s)
{
    ...
    VDPAUContext *ctx = vdpau->hwaccel_ctx;

    av_log(NULL, AV_LOG_INFO, "Closing VDPAU session..");

    vdpau->hwaccel_uninit        = NULL;
    vdpau->hwaccel_get_buffer    = NULL;
    vdpau->hwaccel_retrieve_data = NULL;
...
static int vdpau_retrieve_data(AVCodecContext *s, AVFrame *frame)
{
    ...
    err = ctx->video_surface_get_parameters(surface, &chroma_type,
                                            (uint32_t *) &ctx->tmp_frame->width,
                                            (uint32_t *) &ctx->tmp_frame->height);
    if (err != VDP_STATUS_OK) {
    ...
    err = ctx->video_surface_get_bits(surface, ctx->vdpau_format,
                                      ...
                                      (uint32_t *) ctx->tmp_frame->linesize);
    if (err != VDP_STATUS_OK) {
...
static int vdpau_alloc(AVCodecContext *s)
{
    ...
    vdpau->hwaccel_ctx           = ctx;
    vdpau->hwaccel_uninit        = vdpau_uninit;
    vdpau->hwaccel_get_buffer    = vdpau_get_buffer;
    vdpau->hwaccel_retrieve_data = vdpau_retrieve_data;
    ...
    if (!ctx->dpy) {
        av_log(NULL, AV_LOG_INFO, "Cannot open the X11 display %s.\n",
               XDisplayName(NULL));
        goto fail;
    }
    ...
    if (err != VDP_STATUS_OK) {
        av_log(NULL, AV_LOG_INFO, "VDPAU device creation on X11 display %s failed.\n",
               display);
        goto fail;
    }
...
do {                                                                            \
    ...
    if (err != VDP_STATUS_OK) {                                                 \
        av_log(NULL, AV_LOG_INFO, "Error getting the " #id " callback.\n");        \
        goto fail;                                                              \
    }                                                                           \
    ctx->result = tmp;                                                          \
} while (0)
    ...
    av_log(NULL, AV_LOG_INFO, "Using VDPAU -- %s -- on X11 display %s\n", vendor, display);

    return 0;

fail:
    av_log(NULL, AV_LOG_INFO, "VDPAU init failed\n");
    vdpau_uninit(s);
    return AVERROR(EINVAL);
}
