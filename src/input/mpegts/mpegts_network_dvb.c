...
dvb_network_find_mux
  ( dvb_network_t *ln, dvb_mux_conf_t *dmc, uint16_t onid, uint16_t tsid, int check, int approx )
{
  int deltaf, deltar;
  ...
  LIST_FOREACH(mm, &ln->mn_muxes, mm_network_link) {
    ...
    if (!approx && (lm->lm_tuning.dmc_fe_delsys != dmc->dmc_fe_delsys)) continue;
    ...
    /* DVB-S extra checks */
    if (!approx && (lm->lm_tuning.dmc_fe_type == DVB_TYPE_S)) {
